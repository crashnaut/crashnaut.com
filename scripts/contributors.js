...
;(async () => {
  ...
  for (const slug of posts) {
    ...
    fs.writeFileSync(
      `${content}/${slug}/contributors.json`,
...
async function getContributors(slug) {
  const buffer = execSync(
    `git log --follow --format=%an ${content}/${slug}/index.md`,
  )
  if (!buffer) {
    return []
  }
  ...
  const authors = buffer
    ...
    .filter(
      (a) =>
        !a.toLowerCase().includes("mike") && !a.toLowerCase().includes("sell"),
    )
  if (authors.length === 0) {
    return []
  }
  ...
  try {
    ...
    const navigation = await page.goto(
      `https://github.com/crashnaut/crashnaut.com/commits/main/blog/${slug}/index.md`,
    ...
    while (true) {
      ...
      if (
        await page
          .locator("text=End of commit history for this file")
          .isVisible()
      ) {
        break
      }
