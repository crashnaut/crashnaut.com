...
const startReg = /^:::compare\s*$/;
const endReg = /^:::$/;
const sectionReg = /^::(do|dont)(?:\[(.*?)\])?\s*$/;
...
export const compareBlock = {
	...
	tokenizer(this, src: string, _tokens) {
		...
		const cols: { kind: string; title: string; text: string[]; tokens: unknown[] }[] = [];
		...
		const token = {
			type: 'compareBlock',
			raw,
			cols: cols.map((c) => ({ kind: c.kind, title: c.title, text: c.text.join('\n'), tokens: [] })),
		};
