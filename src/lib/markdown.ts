...
fs.writeFileSync(
  "src/routes/light.theme.css",
  createStyle("light", palleteDawn),
)
...
/** Frontmatter plus fields we mutate while parsing markdown */
export type ParsedMarkdownMetadata = Record<string, unknown> & {
...
export function parseFileToHtmlAndMeta(file): {
  ...
  const { content, metadata: rawMeta } = extractFrontmatter(markdown)
  const metadata = rawMeta as ParsedMarkdownMetadata
  metadata.outgoingSlugs = []
  metadata.toc = []
  ...
  renderer.code = (source, lang) => {
    ...
    const fileIndex =
      lang.indexOf(":") === -1 ? lang.indexOf(" ") : lang.indexOf(":")
    ...
    function generateHTMLFromTokens(tokens: ThemedToken[][]): string {
      ...
      function replaceColorToCSSVariable(color: string) {
        const scopeColors = pallete.tokenColors
          ...
          .map((tc) => {
            return {
              scope: (Array.isArray(tc.scope) ? tc.scope : [tc.scope]).map(
                (c) => c.replace(/\./g, "-"),
              ),
...
function extractFrontmatter(markdown): {
  ...
  if (typeof result.attributes.tags === "string") {
    result.attributes.tags = result.attributes.tags
      .split(",")
      .map((a) =>
        a ? a.trim().charAt(0).toUpperCase() + a.trim().slice(1) : a,
      )
      .map((a) => {
  ...
  if (Array.isArray(result.attributes.translations)) {
    for (const translation of result.attributes.translations) {
      ...
      translation.language =
        translationsMap[translation.language] ?? translation.language
    }
  }
