...
function generate(posts: Awaited<ReturnType<typeof readPosts>>) {
	const nodes = posts.map((post) => {
		...
		const tldrLink = post.tldr
			? `<p><a href="${link}?tldr=true">Read the <strong>TLDR version</strong> on crashnaut.com</a></p>`
			: `<p>Read <strong>${post.metadata.title}</strong> on <a href="${link}">crashnaut.com</a></p>`;
		return {
	...
	<title>Mike Sell - Crashnaut</title>
	${imageLink}
	</image>`;
	const xml = `<?xml version="1.0" encoding="UTF-8" ?>
		...
		<channel>
		<title><![CDATA[ Mike Sell - Crashnaut ]]></title>
		<description><![CDATA[ Blog by Mike Sell (Crashnaut) - QA SDET and Digital Marketing ]]></description>
