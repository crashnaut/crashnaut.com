    ...
    post: {
      ...
      metadata: {
        ...
        toc: post.metadata.toc.filter(
          ({ level, description }) =>
            level < 4 && !description.includes("omit from toc"),
        ),
        ...
        logos: post.metadata.tags
          ...
          .filter(Boolean),
        edit: `https://github.com/crashnaut/crashnaut.com/tree/main/blog/${post.metadata.slug}/index.md`,
      },
      contributors,
      beehiivId: post.metadata.tags.some((x) => x.toLowerCase() == ".net")
        ? "8429a039-a5f6-4056-92f8-b6a53f7b28a3"
        : post.metadata.tags.some(
              (x) => x.toLowerCase() == "angular" || x.toLowerCase() == "ngrx",
            )
